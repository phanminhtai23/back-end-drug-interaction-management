...
class GeminiModel:
    ...
    def pdf_to_DDIs(self, pdf_url):
        try:
            ...
            chat = self.gemini_model.start_chat(history=self.prompt_get_DDIs)
    ...
    def pdf_to_drug_infor(self, pdf_url):
        try:
            # Generate content using the cached prompt and document
            chat = self.gemini_model.start_chat(history=self.prompt_get_drug_infor)
            doc_data = base64.standard_b64encode(
                httpx.get(pdf_url).content
            ).decode("utf-8")
